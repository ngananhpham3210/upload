...
async function openAndInjectInAITab(promptText) {
  // Always create a new tab for individual automations.
  const newTab = await chrome.tabs.create({ url: targetUrl, active: true });

  // Wait for the new tab to finish loading before injecting scripts.
  const listener = (tabId, changeInfo) => {
    if (tabId === newTab.id && changeInfo.status === 'complete') {
      injectAndExecute(tabId, promptText);
      // Clean up the listener to prevent it from running again for this tab.
      chrome.tabs.onUpdated.removeListener(listener);
    }
  };
  chrome.tabs.onUpdated.addListener(listener);
